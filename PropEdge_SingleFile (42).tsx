...
function PropsPage({ liveCtx, isPro }: { liveCtx?: string; isPro: boolean }) {
  ...
  const handleUpgrade = async () => {
    try {
      const res = await fetch(`${BACKEND}/api/stripe/create-checkout`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: "user@elitebetsai.com", userId: "user" }),
      });
      const data = await res.json();
      if (data.url) window.location.href = data.url;
      else alert("Upgrade unavailable — Stripe not configured yet");
    } catch {
      alert("Upgrade unavailable — check back soon!");
    }
  };
...
export default function App() {
  ...
  const handleGlobalUpgrade = async () => {
    try {
      ...
      const data = await res.json();
      if (data.url) window.location.href = data.url;
      else alert("Stripe not configured yet — add STRIPE_PRICE_ID to Render");
    } catch {
      alert("Upgrade unavailable — check back soon!");
    }
  };
