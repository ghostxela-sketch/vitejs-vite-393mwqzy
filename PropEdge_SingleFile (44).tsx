...
const BACKEND = "https://oddsiq-ai.ghostxela.workers.dev"; // Cloudflare Worker - AI only
const RENDER = "https://propedge-backend-m7kn.onrender.com"; // Render - auth, stripe, odds
...
async function proxiedFetch(url: string): Promise<Response> {
  ...
  if (url.startsWith(oddsBase)) {
    try {
      ...
      const renderUrl = `${RENDER}/api/odds/${path}?${params.toString()}`;
...
function buildSystem(ctx: string): string {
  ...
  return `You are EliteBetsAI, an elite sports betting analyst. Today is ${today}.
...
function PropCard({ p, onClick, selected, isPro }: { p:PlayerProp; onClick:()=>void; selected:boolean; isPro:boolean }) {
  const ev = p.ev ?? 0;
  const evColor = ev >= 6 ? C.green : ev >= 3 ? C.accent : ev >= 0 ? C.amber : C.red;
  const sc = (SPORT_CONFIGS as any)[p.sport]?.color ?? C.accent;
  return (
    <div onClick={isPro ? onClick : undefined} style={{
      background: selected ? "#3B82F610" : C.card,
      border: `1px solid ${selected ? C.accent : C.border}`,
      borderRadius: 14, padding: "14px", cursor: isPro ? "pointer" : "default",
      transition: "all 0.15s", position: "relative", overflow: "hidden",
      boxShadow: selected ? `0 0 0 1px ${C.accent}40` : "none",
    }}>
      {/* Player name + team - always visible */}
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
        <div style={{flex:1,minWidth:0,marginRight:8}}>
          ...
          <div style={{fontSize:11,color:C.textDim,marginTop:2}}>{p.team} vs {p.opponent}</div>
        </div>
        {isPro ? <ProbRing prob={p.aiProb}/> : <div style={{width:44,height:44,borderRadius:"50%",background:C.border,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>🔒</div>}
      </div>

      {/* Blur overlay for free users */}
      {!isPro && (
        <div style={{position:"absolute",inset:0,top:60,backdropFilter:"blur(6px)",background:"rgba(8,8,8,0.7)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:8,borderRadius:"0 0 14px 14px",zIndex:10}}>
          <div style={{fontSize:16}}>🔒</div>
          <div style={{fontSize:12,fontWeight:700,color:C.text,textAlign:"center"}}>Pro Required</div>
          <div style={{fontSize:11,color:C.textDim,textAlign:"center",padding:"0 16px"}}>Subscribe to see odds, AI analysis & more</div>
        </div>
      )}
...
// ── Chat ──────────────────────────────────────
function Chat() {
  const [msgs, setMsgs] = useState([{role:"assistant",text:"I'm EliteBetsAI with live player prop odds. OKC leads WCF 2-1, NYK leads ECF 2-0, MTL upset CAR in NHL ECF. Ask me about any player prop, SGP, or tonight's matchups."}]);
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",height:520,background:C.surface,border:`1px solid ${C.border}`,borderRadius:16,overflow:"hidden"}}>
      <div style={{padding:"14px 18px",borderBottom:`1px solid ${C.border}`,background:C.card,display:"flex",alignItems:"center",gap:10}}>
        ...
        <span style={{fontSize:13,fontWeight:700,color:C.text}}>EliteBetsAI</span>
...
function PropsPage({ liveCtx, isPro }: { liveCtx?: string; isPro: boolean }) {
  ...
  const handleUpgrade = async () => {
    try {
      const res = await fetch(`${RENDER}/api/stripe/create-checkout`, {
        ...
        body: JSON.stringify({ email: "guest@elitebetsai.com", userId: "guest" }),
      });
...
const AUTH_KEY = "elitebetsai_user";
const USERS_KEY = "elitebetsai_users";
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:20 }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:40, animation:"fadeIn 0.4s ease" }}>
        ...
        <div>
          <div style={{ fontSize:24, fontWeight:900, color:C.text, letterSpacing:"-0.03em" }}>EliteBetsAI</div>
        ...
        <div style={{ padding:"28px 32px", display:"flex", flexDirection:"column", gap:16 }}>
          ...
          {mode === "signup" && (
            <p style={{ fontSize:11, color:C.textDim, textAlign:"center", lineHeight:1.6 }}>
              By creating an account you agree to our Terms of Service. For entertainment purposes only · EliteBetsAI.
            </p>
          )}
        </div>
      </div>
...
function ProfilePage({ user, onBack, onUpdate }: { user:User; onBack:()=>void; onUpdate:(u:User)=>void }) {
  ...
  const save = async () => {
    ...
    // Update on backend
    try {
      await fetch(`${RENDER}/api/auth/update`, {
        method:"POST", headers:{"Content-Type":"application/json"},
  ...
  return (
    <PageShell title="Settings" icon="⚙️" onBack={onBack}>
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        ...
        {/* About */}
        <div style={{padding:"20px 24px",background:C.surface,borderRadius:14,border:`1px solid ${C.border}`}}>
          <div style={{fontSize:13,fontWeight:700,color:C.textSub,marginBottom:14,textTransform:"uppercase",letterSpacing:"0.06em"}}>About EliteBetsAI</div>
...
const COMMUNITY_KEY = "elitebetsai_community_v1";
    ...
    try {
      const res = await fetch(BACKEND, {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1000,
          system: `You are EliteBetsAI, an expert sports betting analyst. Analyze betting slip images and provide detailed, honest feedback.
...
// ── Upgrade Gate ──────────────────────────────
function UpgradeGate({ onUpgrade, feature }: { onUpgrade: () => void; feature: string }) {
  const perks = [
    "✅ Full player props — odds, lines, EV%",
    "✅ AI analysis on every prop",
    "✅ Live scores & real-time odds",
    "✅ AI Chat with live context",
    "✅ Community chat & pick sharing",
    "✅ AI Pick Analyzer — upload betslips",
    "✅ Kelly criterion & probability rings",
    "✅ All sports: NBA, NFL, NHL, MLB, Soccer",
  ];

  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:400,padding:"40px 20px",textAlign:"center"}}>
      <div style={{maxWidth:480,width:"100%"}}>
        {/* Lock icon */}
        <div style={{fontSize:56,marginBottom:16}}>🔒</div>
        <div style={{fontSize:22,fontWeight:900,color:C.text,marginBottom:8,letterSpacing:"-0.02em"}}>
          {feature} is a Pro Feature
        </div>
        <div style={{fontSize:14,color:C.textSub,marginBottom:24,lineHeight:1.6}}>
          Upgrade to EliteBetsAI Pro to unlock full access to all features
        </div>

        {/* Perks */}
        <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:"20px 24px",marginBottom:24,textAlign:"left"}}>
          <div style={{fontSize:12,fontWeight:700,color:C.textDim,textTransform:"uppercase",letterSpacing:"0.06em",marginBottom:12}}>What you get with Pro</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"8px 16px"}}>
            {perks.map(p=>(
              <div key={p} style={{fontSize:12,color:C.textSub}}>{p}</div>
            ))}
          </div>
        </div>
        ...
        <div style={{marginBottom:20}}>
          <span style={{fontSize:36,fontWeight:900,color:C.text,letterSpacing:"-0.03em"}}>$19.99</span>
          <span style={{fontSize:14,color:C.textDim}}>/month</span>
          <div style={{fontSize:12,color:C.textDim,marginTop:4}}>Cancel anytime · Secure payment via Stripe</div>
        </div>
        ...
        <div style={{fontSize:11,color:C.textDim,marginTop:12}}>
          Free plan: view player names only · Pro: full access to everything
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{background:C.bg,minHeight:"100vh",color:C.text,fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif"}}>
      ...
      <header style={{background:C.surface,borderBottom:`1px solid ${C.border}`,position:"sticky",top:0,zIndex:100}}>
        <div style={{maxWidth:1400,margin:"0 auto",padding:"0 24px",display:"flex",alignItems:"center",gap:16,height:60,flexWrap:"wrap"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div>
              <div style={{fontSize:16,fontWeight:900,color:C.text,letterSpacing:"-0.03em"}}><span style={{color:C.textSub,fontStyle:"italic"}}>Elite</span><span style={{color:C.accent}}>Bets</span><span style={{color:C.green}}>AI</span></div>
      ...
      <main style={{maxWidth:1400,margin:"0 auto",padding:"24px"}}>
        {view==="Props" && <PropsPage liveCtx={liveCtx} isPro={user?.plan==="pro"}/>}
        {view==="Live Odds" && (isPro ? <OddsPage/> : <UpgradeGate onUpgrade={handleGlobalUpgrade} feature="Live Odds"/>)}
        {view==="AI Chat" && (isPro ? <Chat/> : <UpgradeGate onUpgrade={handleGlobalUpgrade} feature="AI Chat"/>)}
        {view==="Community" && (isPro ? <CommunityPage user={user}/> : <UpgradeGate onUpgrade={handleGlobalUpgrade} feature="Community Chat"/>)}
        {view==="Pick Analyzer" && (isPro ? <PickAnalyzerPage liveCtx={liveCtx}/> : <UpgradeGate onUpgrade={handleGlobalUpgrade} feature="AI Pick Analyzer"/>)}
      </main>
      ...
      <footer style={{borderTop:`1px solid ${C.border}`,padding:"16px 24px",marginTop:40}}>
        <div style={{maxWidth:1400,margin:"0 auto",display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
          <span style={{fontSize:11,color:C.textDim}}>EliteBetsAI · For entertainment purposes only</span>
          <span style={{fontSize:11,color:C.textDim}}>AI: Claude Sonnet · Odds: The Odds API · DK / FD / BetMGM · EliteBetsAI v1.0</span>
        </div>
      </footer>
    </div>
  );
